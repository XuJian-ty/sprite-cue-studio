...
export function ensureWolfBossSkillActions(actions: CharacterAction[]): CharacterAction[] {
  const ensureSkill = (id: string, name: string, settings: EnemySkillSettings) => {
    if (actions.some((action) => action.id === id)) return;
    const skill = createAction(name, "skill");
    skill.id = id;
    skill.enemySkill = settings;
    const hurtIndex = actions.findIndex((action) => action.type === "hurt");
    if (hurtIndex >= 0) actions.splice(hurtIndex, 0, skill);
    else actions.push(skill);
  };
  ensureSkill("skill-summon-lightning", "召唤落雷", {
    cooldownSeconds: 7,
    minRange: 2,
    maxRange: 9,
    selectionWeight: 75,
  ...
  ensureSkill("skill-lightning-beam", "雷电光束", {
    cooldownSeconds: 8,
    minRange: 3,
    maxRange: 10,
    selectionWeight: 85,
...
export function createWolfBossBehaviorSettings(current: EnemyBehaviorSettings, actions: CharacterAction[]): EnemyBehaviorSettings {
  const actionId = (id: string, name: string) => actions.find((action) => action.id === id)?.id
    || actions.find((action) => action.name.includes(name))?.id
    || id;
  const chargeId = actionId("skill-charge-knockback", "冲锋撞击");
  ...
  const add = (id: string, name: string, type: EnemyBehaviorNodeType, parentId = "", order = 0) => {
    const node = createBehaviorNode(name, type, parentId, order);
    node.id = id;
    nodes.push(node);
    return node;
  };
  const addTask = (id: string, name: string, taskKey: string, parentId: string, order: number) => {
    const node = add(id, name, "customTask", parentId, order);
    node.taskKey = taskKey;
    return node;
  };
  const addAction = (id: string, name: string, action: string, parentId: string, order: number) => {
  ...
  const addCombo = (index: number, name: string, cooldown: number) => {
    const cooldownNode = add(`wolf-combo-${index}-cooldown`, `${name}冷却`, "cooldown", "wolf-combo-random", index - 1);
    cooldownNode.durationSeconds = cooldown;
    const sequence = add(`wolf-combo-${index}`, name, "sequence", cooldownNode.id, 0);
    const target = add(`wolf-combo-${index}-target`, "已发现目标", "condition", sequence.id, 0);
    target.conditionKey = "hasTarget";
    target.comparison = "isTrue";
    addTask(`wolf-combo-${index}-stop`, "连招前停止", "stop", sequence.id, 1);
    addTask(`wolf-combo-${index}-face`, "面向玩家", "faceTarget", sequence.id, 2);
    return sequence;
  };
  ...
  const combo1 = addCombo(1, "冲锋接四连挥爪", 6);
  addAction("wolf-combo-1-charge", "冲锋撞击", chargeId, combo1.id, 3);
  addAction("wolf-combo-1-claw", "四连挥爪", clawId, combo1.id, 4);

  const combo2 = addCombo(2, "隐身背袭接四连挥爪", 6);
  addAction("wolf-combo-2-ambush", "隐身背袭", ambushId, combo2.id, 3);
  addAction("wolf-combo-2-claw", "四连挥爪", clawId, combo2.id, 4);

  const combo3 = addCombo(3, "背袭冲锋接四连挥爪", 8);
  addAction("wolf-combo-3-ambush", "隐身背袭", ambushId, combo3.id, 3);
  addAction("wolf-combo-3-charge", "冲锋撞击", chargeId, combo3.id, 4);
  addAction("wolf-combo-3-claw", "四连挥爪", clawId, combo3.id, 5);

  const combo4 = addCombo(4, "三次往返冲锋", 8);
  addAction("wolf-combo-4-charge-1", "第一次冲锋", chargeId, combo4.id, 3);
  addAction("wolf-combo-4-teleport-1", "第一次转向前瞬移", teleportId, combo4.id, 4);
  addTask("wolf-combo-4-turn-1", "第一次转向", "turnAround", combo4.id, 5);
  addAction("wolf-combo-4-charge-2", "第二次冲锋", chargeId, combo4.id, 6);
  addAction("wolf-combo-4-teleport-2", "第二次转向前瞬移", teleportId, combo4.id, 7);
  addTask("wolf-combo-4-turn-2", "第二次转向", "turnAround", combo4.id, 8);
  addAction("wolf-combo-4-charge-3", "第三次冲锋", chargeId, combo4.id, 9);

  const combo5 = addCombo(5, "隐身背袭接雷电光束", 8);
...
export function ensureWolfBossCombo4TeleportSteps(current: EnemyBehaviorSettings, actions: CharacterAction[]): EnemyBehaviorSettings {
  const teleportAction = actions.find((action) => action.id === "skill-teleport")
    || actions.find((action) => action.name.includes("瞬移"));
  if (!teleportAction || !current.nodes.some((node) => node.id === "wolf-combo-4")) return current;

  const firstTeleportId = "wolf-combo-4-teleport-1";
  const secondTeleportId = "wolf-combo-4-teleport-2";
  const alreadyConfigured = current.nodes.some((node) => node.id === firstTeleportId && node.actionId === teleportAction.id)
    && current.nodes.some((node) => node.id === secondTeleportId && node.actionId === teleportAction.id);
  if (alreadyConfigured) return current;

  const next = structuredClone(current);
  const byId = new Map(next.nodes.map((node) => [node.id, node]));
  const charge1 = byId.get("wolf-combo-4-charge-1");
  const baseX = charge1?.positionX || 0;
  const baseY = charge1?.positionY || 0;
  const upsertTeleport = (id: string, name: string, order: number, positionX: number) => {
    let node = byId.get(id);
    if (!node) {
      node = createBehaviorNode(name, "playAction", "wolf-combo-4", order);
      node.id = id;
  ...
  const updateStep = (id: string, order: number, positionX: number) => {
  ...
  upsertTeleport(firstTeleportId, "第一次转向前瞬移", 4, baseX + 230);
  updateStep("wolf-combo-4-turn-1", 5, baseX + 460);
  updateStep("wolf-combo-4-charge-2", 6, baseX + 690);
  upsertTeleport(secondTeleportId, "第二次转向前瞬移", 7, baseX + 920);
